...
function renderChecklist(){
  ...
  checklistItems.forEach((item,i)=>{
    ...
    label.innerHTML=`<input type="checkbox" data-i="${i}" ${checklistState[i]?'checked':''}><span>${item}</span>`;
    checklist.appendChild(label);
  });
  const done=checklistItems.filter((_,i)=>checklistState[i]).length;
  $('checkProgress').textContent=Math.round(done/checklistItems.length*100);
}
checklist.addEventListener('change',e=>{
  if(e.target.matches('input')){
    checklistState[e.target.dataset.i]=e.target.checked;
...
const trackerForm=$('trackerForm'),list=$('applicationList'),dateInput=$('dateApplied');
dateInput.value=new Date().toISOString().slice(0,10);
function saveApps(){localStorage.setItem(appKey,JSON.stringify(applications));renderApps()}
function renderApps(){
  $('applicationCount').textContent=`${applications.length} application${applications.length===1?'':'s'}`;
  if(!applications.length){list.innerHTML='<div class="empty">No applications yet. Add your first one above.</div>';return}
  list.innerHTML=applications.map(a=>`<div class="application"><b>${escapeHtml(a.company)}</b><span>${escapeHtml(a.role)}</span><span class="status">${escapeHtml(a.status)}</span><small>${escapeHtml(a.date)}</small><button class="delete" data-id="${a.id}" aria-label="Delete application">×</button></div>`).join('');
}
...
list.addEventListener('click',e=>{
  if(e.target.matches('.delete')){
    applications=applications.filter(a=>String(a.id)!==e.target.dataset.id);
    saveApps();
  }
});
$('clearApplications').addEventListener('click',()=>{
  if(applications.length&&confirm('Clear all saved applications on this device?')){
    applications=[];
    saveApps();
  }
});
...
const roadmapKey='lam-roadmap';
let roadmap=JSON.parse(localStorage.getItem(roadmapKey)||'{}');
const grid=$('roadmapGrid');
function renderRoadmap(){
  grid.innerHTML=modules.map((m,i)=>`<article class="roadmap-card ${roadmap[i]?'done':''}"><label><input type="checkbox" data-r="${i}" ${roadmap[i]?'checked':''}><span><b>${m[0]} ${m[1]}</b><small>${m[2]}</small></span></label></article>`).join('');
}
grid.addEventListener('change',e=>{
  if(e.target.matches('[data-r]')){
    roadmap[e.target.dataset.r]=e.target.checked;
